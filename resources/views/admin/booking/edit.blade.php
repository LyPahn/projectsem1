    ...
    <section class="content">
     ...
     <div class="col-md-12">
          ...
          <div class="box box-primary">
            ...
            <!-- /.box-header -->
            <!-- form start -->
            <form role="form" method="POST" enctype="multipart/form-data" action="{{route('booking.update',$booking)}}">
                @csrf
                @method("PUT")
                <div class="box-body">
                  <div class="form-group">
                    ...
                    <select name="user_id" id="input" class="form-control">
                      @foreach ($users as $item)
                          <option value="{{$item->id}}" {{($booking->user_id === $item->id)?'selected':''}}>{{$item->name}}</option>
                      @endforeach
                    </select>
                  </div>
                  <div class="form-group">
                  ...
                  <div class="form-group">
                    ...
                    <div class="radio">
                      <label>
                        <input type="radio" name="status" id="input" value="1" {{$booking->status ? 'checked' : ""}}>
                        1
                      </label>
                      <label>
                        <input type="radio" name="status" id="input" value="0" {{!$booking->status ? 'checked' : ""}}>
                        0
                      </label>
                    </div>
                  </div>
                  <div class="row">
                    <div class="col-md-6">
                      <div class="form-group">
                        ...
                        <input type="datetime" name="check_in" class="form-control" value="{{old('check_in') ? old('check_in') : $booking->check_in}}">
                      </div>
                    </div>
                    <div class="col-md-6">
                      <div class="form-group">
                        <label for="exampleInputEmail1">check_out</label>     
                        <input type="datetime" name="check_out" class="form-control" value="{{old('check_out') ? old('check_out') : $booking->check_out}}">
